...
public interface ReminderRepository extends CrudRepository<Reminder, Long> {
    ...
    @Query("""
            WITH all_1_reminder as (
            select distinct outbox_msg_id 
            from reminder
            where is_target =1
            and supplier_corp_id = :supplierCorpId
            and type = 1
            ...
            select * 
            from reminder
            where outbox_msg_id not in (select * from all_1_reminder)
            and is_target = 1
            and type = 2
            and supplier_corp_id = :supplierCorpId
            order by sort_col, outbox_msg_id
    ...
    @Query("""
            ...
            select * 
            from reminder
            where outbox_msg_id not in (select * from all_1_2_reminder)
            and is_target = 1
            and type = 3
            and supplier_corp_id = :supplierCorpId
            order by sort_col, outbox_msg_id
